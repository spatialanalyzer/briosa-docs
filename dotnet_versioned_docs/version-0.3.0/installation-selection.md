...
description: Client 0.3.0 server selectors, inert discovery, selection reports, and startup failures for SA 2026.1.0529.7.
---
...
These APIs are released in client **0.3.0**. Import them from the same exact-target package as the client. Discovery reads local installation evidence without launching the server, SDK, or SpatialAnalyzer. It does not prove runtime readiness.
...
var report = BriosaInstallations.Discover(new BriosaServerSelection { MinimumVersion = "0.8.0" });
...
[Released Source](https://github.com/spatialanalyzer/briosa-dotnet/blob/v0.3.0/targets/2026.1.0529.7/src/Briosa.Client/BriosaServerSelection.cs)
