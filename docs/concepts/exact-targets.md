...
4. Start Briosa's disconnected SDK generation
5. Start the matching application yourself or ask Briosa to launch it
...
3. Stop the current exact-target Briosa server and start the matching distribution
4. Start a new disconnected SDK generation
5. Start the intended SpatialAnalyzer release or ask that Briosa server to launch it
6. Connect the SDK and require exact-match readiness
...
- The client reports that connection or readiness could not be established
- No command is sent merely to test whether the mismatched combination happens
  to work

Select the correct Briosa distribution or application, correct the SDK
registration when necessary, replace the SDK if required, and then connect it.
...
Server 0.8.0 and clients 0.3.0 are released separately for SpatialAnalyzer `2024.1.0508.5` and `2026.1.0529.7`. Use the [SA 2024 reference](/api/grpc/sa-2024.1.0508.5) or [SA 2026 reference](/api/grpc). The SA 2026
...
[Briosa server repository](https://github.com/spatialanalyzer/briosa/tree/v0.8.0/targets).
